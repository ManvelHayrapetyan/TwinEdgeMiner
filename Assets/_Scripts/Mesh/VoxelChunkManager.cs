...
public class VoxelChunkManager : MonoBehaviour
{
    ...
    [SerializeField] private int _voxelsPerChunkX = 16;
    [SerializeField] private int _voxelsPerChunkY = 16;
    [SerializeField] private int _voxelsPerChunkZ = 16;
    ...
    private void Awake()
    {
        _chunkWorldSize = new Vector3(_voxelsPerChunkX - 1, _voxelsPerChunkY - 1, _voxelsPerChunkZ - 1) * _voxelSize;
        CreateChunks();
    }

    public void ApplyDamage(Vector3 worldPosition, float radius, float stabilityDamage, float durabilityDamage)
    {
        List<Vector3Int> affectedChunks = GetAffectedChunks(worldPosition, radius);
        List<Vector3Int> affectedOreChunks = new();
        Dictionary<Vector3Int, Color[]> indexToColor = new();
            ...
            foreach (Vector3Int chunkIndex in _oreToChunkList[oreIndex])
            {
                switch (oreDamageResult)
                {
                    ...
                    case OreDamageResult.CrackChanged:
                        ...
                        Color[] crackColors = _chunkDict[chunkIndex].ApplyCrack(
                            ...
                            oreIndex);
                        indexToColor[chunkIndex] = crackColors;
        ...
        affectedChunks.AddRange(affectedOreChunks);
        affectedChunks.Distinct();
        ...
        UpdateChunks(affectedChunks.ToArray(), indexToColor);
    }
    ...
    private void UpdateChunks(Vector3Int[] indexes, Dictionary<Vector3Int, Color[]> indexToColor)
    {
        Debug.Log($"Chunk Update count {indexes.Length}");
        foreach (Vector3Int index in indexes)
        {
            _chunkRendererDict[index].UpdateGO(_chunkDict[index]);
            _chunkRendererDict[index].UpdateMesh();
            if (indexToColor.TryGetValue(index, out Color[] color))
    ...
    public void OreGroundInitialize(OreMineable oreMinable, int index)
    {
        ...
        List<Vector3Int> affectedChunks = GetAffectedChunks(center, radius);
    ...
    private List<Vector3Int> GetAffectedChunks(Vector3 center, float radius)
    {
